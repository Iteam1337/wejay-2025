...
async function updateMembership(ctx, action) {
...
const joinRoom = (ctx) => updateMembership(ctx, 'join');
const leaveRoom = (ctx) => updateMembership(ctx, 'leave');
const SPOTIFY_API = 'https://api.spotify.com/v1';
async function spotifyFetch(accessToken, endpoint, options) {
...
async function createSpotifyPlaylist(accessToken, userId, roomName) {
...
async function syncQueueToPlaylist(accessToken, playlistId, roomId) {
    ...
    if (!queue.tracks?.length)
        return;
    const uris = queue.tracks.map((t) => `spotify:track:${t.spotifyId}`);
...
export function roomsApiMiddleware() {
