...
function scheduleVerifierCleanup(state: string) {
  setTimeout(() => {
...
function setAuthCookies(ctx: Router.RouterContext, data: TokenResponse, includeRefresh = true) {
...
function clearAuthCookies(ctx: Router.RouterContext) {
  const cookies = [
...
function exchangeCodeForToken(code: string, redirectUri: string, codeVerifier: string) {
...
async function storeVerifier(ctx: Router.RouterContext) {
...
function validateExchangeRequest(body: ExchangeRequest): ValidationResult {
  const missing = [
    !body.code && 'code',
...
function validateVerifier(stored: VerifierData | undefined, state: string): ValidationResult {
  if (!stored) {
...
function validateCredentials(): ValidationResult {
  const clientId = process.env.VITE_SPOTIFY_CLIENT_ID;
  const clientSecret = process.env.CLIENT_SECRET;
...
async function exchangeToken(ctx: Router.RouterContext) {
  const body = ctx.request.body as ExchangeRequest;
...
async function getToken(ctx: Router.RouterContext) {
...
async function refreshToken(ctx: Router.RouterContext) {
...
async function logout(ctx: Router.RouterContext) {
  clearAuthCookies(ctx);
  ctx.body = { success: true };
}

export function authMiddleware(): Router.Middleware {
